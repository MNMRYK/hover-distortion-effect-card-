...
// Seleccionamos TODOS los wrappers y aplicamos la lógica a cada uno
document.querySelectorAll(".wrapper").forEach((wrapper) => {
  
  // 1. Efecto de fondo individual
  // Extraemos las URLs del HTML de ESTE wrapper específico
  const imgUno = wrapper.getAttribute('data-img1');
  const imgDos = wrapper.getAttribute('data-img2');
  ...
  new hoverEffect({
    ...
    image1: imgUno,
    image2: imgDos,
    displacementImage: "https://i.postimg.cc/QNTRDRks/4.png" // Esta puede ser la misma para todos
  });

  // 1. Seleccionamos el contenedor del avión para tratarlo como una unidad
  const plane = wrapper.querySelector('.plane-icon'); // USA wrapper, NO document
  if (plane) {
    plane.classList.add('letter');
  }

  wrapper.querySelectorAll('.hero-title h1, .hero-title .date').forEach(el => {
    el.innerHTML = el.textContent.replace(/\S/g, "<span class='letter'>$&</span>");
  });

  // 3. Animación específica para este wrapper al pasar el ratón
  $(wrapper).hover(
    function () {
...
// https://i.postimg.cc/g2Y5vr9k/6.jpg
// https://i.postimg.cc/1XwTKq54/7.jpg
// https://i.postimg.cc/YqBTCbbF/8.jpg
// https://i.postimg.cc/KYjHHVv6/9.jpg

document.querySelectorAll(".wrapper").forEach((wrapper) => {
  
  // ... (Aquí va el código de hoverEffect y de separar letras que ya tienes)

  // 3. Animación específica para CADA wrapper
  $(wrapper).hover(
    function () {
      anime.timeline({ loop: false }).add({
        // ✅ CLAVE: Usamos 'wrapper.querySelectorAll' para que solo afecte a sus hijos
        targets: wrapper.querySelectorAll(".letter"), 
        translateX: [80, 0],
        translateZ: 0,
        opacity: [0, 1],
